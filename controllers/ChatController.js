...
async loadChatData(userId, otherUserId, socket) {
  try {
    ...
    if (!meDb) return socket.emit('socketError', { message: 'Your account was not found.' });
    if (!otherUserDb) return socket.emit('socketError', { message: 'The user you’re trying to chat with was not found.' });

    // 2️⃣ Get block status
    ...
    const iBlockedOther = iBlockedEntry ? 1 : 0;
    const blockedByOther = blockedByEntry ? 1 : 0;

    // 3️⃣ Load messages
    const rawMessages = await Chat.findAll({
      where: {
        [Op.and]: [
          {
            [Op.or]: [
    ...
    // 4️⃣ Transform each message to match iOS model
    ...
    // return for joinChat success
    return { messages, user: otherUser, iBlockedOther, blockedByOther };
...
socket.on('joinChat', async ({ userId, otherUserId }) => {
  try {
    ...
    // ✅ Load properly formatted chat data
    const chatData = await this.loadChatData(userId, otherUserId, socket);

    socket.emit('joinChatSuccess', {
      success: true,
      message: 'Joined chat room successfully.',
      room: roomName,
      ...chatData, // contains messages, user, iBlockedOther, blockedByOther
    });

  } catch (error) {
    console.error('Error in joinChat:', error);
    socket.emit('socketError', { message: 'Failed to join chat.', error: error.message });
  }
});

...
// === Accept both image and video fields ===
...
router.post('/upload', (req, res) => {
  ...
  files.image = `${req.protocol}://${req.get('host')}/uploads/${image.filename}`;
    }
    if (req.files.video) {
      const video = req.files.video[0];
      files.video = `${req.protocol}://${req.get('host')}/uploads/videos/${video.filename}`;
    }
